...
const updateReadingProgress = async (progress: ReadingProgress) => {
  ...
  const { error } = await supabase
    ...
    .update({
      // Use the field names you expect in your table. In this case,
      // we update current_paragraph, started_at, last_updated_at, completed_at, total_time_spent, and status.
      current_paragraph: paragraph_no,
      paragraph_id,
      last_updated_at,
      // completed_at,
