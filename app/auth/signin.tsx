...
const signin = () => {
	...
	return (
		...
		>
			...
			<TextInput
				className={`w-10/12 h-14 text-lg text-left px-4 ${isEmailFocused ? 'border-b' : ''}`}
				style={{
			...
			/>
			<TextInput
				className={`w-10/12 h-14 text-lg text-left px-4 ${isPasswordFocused ? 'border-b' : ''}`}
				style={{
					color: theme === 'light' ? light.text : dark.text,
			...
			<View className='w-10/12 py-2'>
				<Pressable
					...
					className='rounded-full w-full p-2 border'
					style={{
